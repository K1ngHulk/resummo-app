...
export async function persistNotionExportModel(client, model, { replaceEditorial = false } = {}) {
...
export async function validateNotionExportPersistence(client, { assetConcurrency = 1 } = {}) {
  ...
  const missingAssetFiles = (await findMissingContentAssets(
    [...assetUrls].map((url) => path.basename(url)),
...
export async function importNotionExportBuffer(buffer, { archiveName, client, replaceEditorial = false }) {
  ...
  try {
    ...
    const persistence = await persistNotionExportModel(client, model, { replaceEditorial })
